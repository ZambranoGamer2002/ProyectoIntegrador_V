<?php 
	if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        CURLOPT_URL => 'https://cevicherias.informaticapp.com/reservas/'.$_POST['res_id'],
                        ...
                        <div class="card mb-4">
							...
							<form method="post">
								    <div class="mb-3">
                                        ...
                                        <input type="hidden" name="per_id" value="<?= $data["Detalles"][0]['res_id'] ?>">
                                        <input type="text" name="per_nombres" id="fecha" value="<?= date('Y-m-d') ?>" required class="form-control" aria-describedby="emailHelp">
                                        <input type="text" name="per_nombres" value="<?= $data["Detalles"][0]['res_autor'] ?>" class="form-control" aria-describedby="emailHelp">
                                    </div>
                                    <div class="mb-3">
                                        <label for="exampleInputEmail1" class="form-label">Apellidos</label>
                                        <input type="text" name="per_apellidos" value="<?= $data["Detalles"][0]['per_apellidos'] ?>" class="form-control" aria-describedby="emailHelp">
                                    </div>
                                    <div class="mb-3">
                                        <label for="exampleInputEmail1" class="form-label">Telefono</label>
                                        <input type="text" name="per_telefono" value="<?= $data["Detalles"][0]['per_telefono'] ?>" class="form-control" aria-describedby="emailHelp">
                                    </div>
                                    <div class="mb-3">
                                        <label for="exampleInputEmail1" class="form-label">DNI</label>
                                        <input type="text" name="per_dni" value="<?= $data["Detalles"][0]['per_dni'] ?>" class="form-control" aria-describedby="emailHelp">
                                    </div>
                                    <div class="mb-3">
                                        <label for="exampleInputEmail1" class="form-label">Correo</label>
                                        <input type="text" name="per_correo" value="<?= $data["Detalles"][0]['per_correo'] ?>" class="form-control" aria-describedby="emailHelp">
                                    </div>
                                    <div class="mb-3">
                                        <label for="exampleInputPassword1"  class="form-label">Sueldo</label>
                                        <input type="text" name="tra_sueldo" value="<?= $data["Detalles"][0]['tra_sueldo'] ?>" class="form-control">
                                    </div>

									<div class="mb-3">
                                        <label for="exampleInputEmail1" class="form-label">Seleccionar tipo de trabajador</label>
                                        <select name="titra_id" class="form-select form-select-sm" aria-label=".form-select-sm example" >
                                        	<option type="text" value="<?= $data["Detalles"][0]['titra_id'] ?>"><?= $data["Detalles"][0]['titra_rol'] ?> - Seleccionado</option>
